...
/**
 * Print Job Management - handles creation, status updates, and retries.
 * Implements use cases for managing print job lifecycle.
 */
@Service
@RequiredArgsConstructor
public class PrintJobManagementService {
  private static final Logger log = LoggerFactory.getLogger(PrintJobManagementService.class);
  ...
  public PrintJobResponse create(CreatePrintJobRequest request) {
    ...
    log.info("PrintJobManagementService.create: idempotencyKey={} sessionId={} documentType={}",
    ...
    try {
      ...
      log.error("PrintJobManagementService.create: unexpected error for idempotencyKey={}: {}",
  ...
  private PrintJobResponse get(UUID id) {
    PrintJob job = getJobOrThrow(id);
    return toResponse(job);
  }

  private PrintJob getJobOrThrow(UUID id) {
    return printJobRepository
        .findByIdAndCompanyId(id, TenantContext.getTenantId())
        .orElseThrow(() -> new OperationException(HttpStatus.NOT_FOUND, "Print job no encontrado"));
  }
