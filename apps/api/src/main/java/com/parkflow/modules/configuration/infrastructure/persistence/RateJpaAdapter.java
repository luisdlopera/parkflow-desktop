...
@Component("configRateJpaAdapter")
...
public class RateJpaAdapter implements RatePort {
  ...
  interface RateJpaRepository extends JpaRepository<Rate, UUID> {
    @Query("SELECT r FROM ConfigRate r WHERE r.companyId = :cid AND (:site IS NULL OR r.site = :site OR r.site IS NULL) "
    ...
    @Query("SELECT r FROM ConfigRate r WHERE r.companyId = :cid AND r.site = :site AND r.isActive = true AND r.id <> :excludeId "
