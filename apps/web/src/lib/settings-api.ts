...
export type ThemeColorPayload = Omit<ThemeConfig, "id" | "companyId" | "logoUrl" | "faviconUrl" | "createdAt" | "updatedAt">;
...
export async function fetchThemeConfig(companyId: string): Promise<ThemeConfig> {
  return apiFetch<ThemeConfig>(`${themeCfgBase()}?companyId=${encodeURIComponent(companyId)}`, {
    cache: "no-store",
    headers: await buildApiHeaders()
  });
}
...
export async function uploadThemeLogo(companyId: string, file: File): Promise<ThemeConfig> {
  ...
  return apiFetch<ThemeConfig>(`${themeCfgBase()}/logo?companyId=${encodeURIComponent(companyId)}`, {
    method: "POST",
    headers: await buildApiHeaders(),
    body: form
  });
...
export async function uploadThemeFavicon(companyId: string, file: File): Promise<ThemeConfig> {
  const form = new FormData();
  form.append("file", file);
  return apiFetch<ThemeConfig>(`${themeCfgBase()}/favicon?companyId=${encodeURIComponent(companyId)}`, {
    method: "POST",
    headers: await buildApiHeaders(),
    body: form
  });
