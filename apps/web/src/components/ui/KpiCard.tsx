...
export default function KpiCard({
  ...
  const getTrendColor = () => {
    if (trendValue === undefined) return "text-default-500";
  ...
  const getStatusColor = () => {
    ...
    return "border border-default-200 dark:border-default-700";
  };

  return (
    <div className={`surface rounded-2xl p-4 sm:p-5 border transition-all duration-300 ease-out hover:border-default-300 dark:hover:border-default-600 hover:bg-default-50/50 dark:hover:bg-default-900/50 hover:-translate-y-1 ${getStatusColor()} relative`}>
      {status === "critical" && (
      ...
      <p className="text-[10px] sm:text-xs uppercase tracking-[0.2em] sm:tracking-[0.3em] text-default-400 truncate">
        {title}
      </p>
      <div className="mt-2 sm:mt-3 flex items-end justify-between gap-2">
        <p className="text-xl sm:text-2xl font-semibold text-foreground dark:text-default-200 truncate">{value}</p>
      </div>
      ...
      {trendValue !== undefined && (
        <div className={`mt-2 flex items-center gap-1 text-xs sm:text-sm font-medium ${getTrendColor()}`}>
          ...
          <span className="text-default-400 text-xs">{trendLabel}</span>
        </div>
      )}

      {/* Fallback trend text */}
      {!trendValue && trend ? (
        <p className="text-xs sm:text-sm font-medium text-default-500 dark:text-default-400 mt-2">{trend}</p>
      ) : null}
    </div>
  );
}
