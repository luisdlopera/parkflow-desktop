  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Facturas Electrónicas</h1>
          <p className="mt-1 text-sm text-slate-500">{total} facturas en total</p>
        </div>
      ...
      <div className="rounded-xl border border-default-200 bg-white">
        {loading ? (
          ...
          <div className="flex h-48 flex-col items-center justify-center gap-2">
            <p className="text-sm font-medium text-slate-600">No hay facturas</p>
            <p className="text-xs text-slate-400">Las facturas aparecen aquí cuando se generan automáticamente o de forma manual</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-default-200 text-left">
                <th className="px-5 py-3 font-semibold text-slate-600">Número</th>
                <th className="px-5 py-3 font-semibold text-slate-600">Externo</th>
                <th className="px-5 py-3 font-semibold text-slate-600">Proveedor</th>
                ...
                <th className="px-5 py-3 font-semibold text-slate-600">Fecha</th>
                <th className="px-5 py-3 font-semibold text-slate-600">Acciones</th>
              </tr>
            </thead>
            <tbody>
              {invoices.map((inv) => (
                <tr key={inv.id} className="border-b border-default-100 last:border-0 hover:bg-slate-50">
                  <td className="px-5 py-3 font-mono text-xs font-semibold text-slate-800">{inv.number}</td>
                  <td className="px-5 py-3 text-xs text-slate-500">{inv.externalNumber ?? "—"}</td>
                  <td className="px-5 py-3 text-xs text-slate-600">{PROVIDER_LABELS[inv.providerType] ?? inv.providerType}</td>
                  <td className="px-5 py-3 font-semibold text-slate-800">{fmtCurrency(inv.total, inv.currency)}</td>
                  <td className="px-5 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${STATUS_CHIP[inv.status]}`}>
                      {STATUS_LABELS[inv.status]}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-xs text-slate-500">{fmtDate(inv.createdAt)}</td>
                  <td className="px-5 py-3">
      ...
      {total > 20 && (
        <div className="flex items-center justify-center gap-2">
          <button
            disabled={page === 0}
            onClick={() => load(page - 1)}
            className="rounded-lg border border-default-200 px-3 py-1.5 text-xs font-semibold text-slate-700 disabled:opacity-40 hover:bg-slate-50"
          >
            Anterior
          </button>
          <span className="text-xs text-slate-500">Página {page + 1} de {Math.ceil(total / 20)}</span>
          <button
      ...
      {showCancelModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="w-full max-w-md rounded-xl border border-default-200 bg-white p-6">
            <h3 className="font-semibold text-slate-900">Anular factura {showCancelModal.number}</h3>
            <p className="mt-1 text-sm text-slate-500">
              Esta acción notificará al proveedor de facturación para anular el documento electrónico.
            </p>
            <div className="mt-4">
              <label className="mb-1 block text-xs font-semibold text-slate-700">Motivo de anulación</label>
              <textarea
                value={cancelReason}
