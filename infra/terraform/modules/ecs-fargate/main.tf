...
# Allow task to read the DB password secret
...
resource "aws_ecs_task_definition" "api" {
  ...
  cpu                      = "256" # Minimal for dev
  memory                   = "512" # Minimal for dev
  ...
  container_definitions = jsonencode([
    {
      ...
      secrets = [
        { name = "SPRING_DATASOURCE_PASSWORD", valueFrom = var.db_password_arn }
      ]
      logConfiguration = {
        logDriver = "awslogs"
        options = {
...
resource "aws_security_group" "alb" {
  ...
  # Add HTTPS 443 here when certificate is ready
...
resource "aws_lb_target_group" "api" {
  ...
  health_check {
    ...
    timeout             = 5
    interval            = 30
    path                = "/actuator/health" # Spring Boot actuator path
    matcher             = "200"
  }
}
...
resource "aws_ecs_service" "main" {
  ...
  desired_count   = 1 # 1 for dev, scale up for prod
  launch_type     = "FARGATE"

  network_configuration {
    security_groups  = [aws_security_group.ecs_tasks.id]
    subnets          = var.private_subnet_ids
    assign_public_ip = false # Security: Tasks are in private subnets
  }
